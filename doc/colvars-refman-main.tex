...
\cvsubsubsec{Treatment of grid boundaries}{sec:colvarbias_meta_boundaries}

In typical scenarios the Gaussian hills of a metadynamics potential are interpolated and summed together into a grid, which is much more efficient than computing each hill independently at every step (the keyword \refkey{useGrids}{metadynamics|useGrids} is \texttt{on} by default).
This numerical approximation typically yields neglibile errors in the resulting PMF \cite{Fiorin2013}.
However, due to the finite thickness of the Gaussian function the metadynamics potential suddenly vanishes each time a variable exceed the chosen grid boundaries.
...
This measure is taken to protect the accuracy and stability of a metadynamics simulation, except in the case of ``natural'' boundaries (for example, the $[0:180]$ interval of an \texttt{angle} colvar) or when the flags \refkey{hardLowerBoundary}{colvar|hardLowerBoundary} and \refkey{hardUpperBoundary}{colvar|hardUpperBoundary} are set by the user to guarantee that the corresponding variable will never exceed its boundaries.
At the same time, the use of explicit hills alongside the more compact potential and force grids can negatively affect simulation speed and memory requirements.

In general, it is a good idea to \emph{define a repulsive potential to prevent any hills from straddling the grid's boundaries}, for example a \texttt{harmonicWalls} restraint (see \ref{sec:colvarbias_harmonic_walls}).\\

\noindent\textbf{Example:} Using walls to protect the grid's boundaries.
{\noindent\ttfamily
\\
\-colvar \{\\
\-\-~~name r\\
...
In the colvar \texttt{r}, the \texttt{distance} function used has a \texttt{lowerBoundary} automatically set to 0~\AA{} by default: the keyword \texttt{lowerBoundary} itself is not mandatory, and \texttt{hardLowerBoundary} is likewise set to \texttt{yes} internally.
However, \texttt{upperBoundary} does not have such a ``natural'' choice of value and needs to be provided in the input.

It goes without saying that if \texttt{r} is a distance between two freely-moving molecules, it will eventually exceed the set \texttt{upperBoundary} of 15~\AA.
The \texttt{meta\_r} bias will track individually any hill whose center lies within fewer grid points than 6 times the Gaussian $\sigma$ parameter, plus one.
In the example above, where \texttt{hillWidth} = 2 grid points, the lower threshold of this ``buffer'' is 6+1 = 7 grid points away from \texttt{upperBoundary}, or equivalently 15 - 7$\times$0.2 = 13.6~\AA.
This threshold would normally lead to a large number of Gaussian hills accumulating as the simulation progresses and \texttt{r} explores the region above 13.6~\AA.
These hills would increase the state file's size, and decrease the simulation speed any time \texttt{r} goes off the grid and the hills are computed individually.

The \texttt{wall\_r} bias also included in the example prevents this: the position of its \texttt{upperWall} is 13~\AA{}, i.e.{} 3 grid points below the buffer's threshold.
Likewise, for the chosen value of \texttt{upperWallConstant}, the energy of the \texttt{wall\_r} bias at the buffer's threshold (13.6~\AA) is:
\begin{equation*}
  E(\mathrm{wall\_r}) = \frac{1}{2} \- k \left(\frac{\xi - \xi_{\mathrm{upper}}}{w_{\xi}}\right)^2 = \frac{1}{2} \- 2.0 \left(-3\right)^2 = 9~\mathrm{kcal/mol}
\end{equation*}
which results in a relative probability $\exp(-E(\mathrm{wall\_r})/\kappa_{\mathrm{B}}T) \simeq$ $3\times{}10^{-7}$ that \texttt{r} enters the buffer region.
The probability that \texttt{r} even leaves the grid has also become vanishingly small: at that point, you may want to set \texttt{hardUpperBoundary} to \texttt{yes} for \texttt{r}, and let \texttt{meta\_r} know that no special treatment near the grid's boundaries will be needed.

\emph{What is the impact of the added walls onto the PMF?} Not a very complicated one: the PMF reconstructed by metadynamics will simply show a sharp increase in free-energy where the wall potential kicks in.
You could choose between using the PMF only up until the wall's position (13~\AA in the example) and discard the rest, or subtracting the energy of the \texttt{harmonicWalls} restraint using its formula (see~\ref{sec:colvarbias_harmonic_walls}).
Keep in mind, however, that the statistical convergence of metadynamics in the region where the wall potential is acting may be less accurate.

In summary, although it would be simpler to set the grid's boundary at the wall's position at the same number, the finite width of the Gaussian hills calls for the use of repulsive walls that are well within the grid's boundaries.
